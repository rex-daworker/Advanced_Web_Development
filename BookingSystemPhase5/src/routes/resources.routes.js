...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    await logEvent({
      actorUserId,
      message: `RESOURCE_CREATED: name="${resourceName}", description="${resourceDescription || '—'}", available=${resourceAvailable}, price=${resourcePrice}, unit="${resourcePriceUnit}"`,
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      ...
      await logEvent({
        actorUserId,
        message: `RESOURCE_CREATE_BLOCKED_DUPLICATE: name="${resourceName}" already exists (409)`,
        entityType: "resource",
        entityId: null,
      });
